...
class MotionLabel: CharacterLabel {
    ...
    func animateIn(_ completion: ((_ finished: Bool) -> Void)? = nil) {
        ...
        for textLayer in characterTextLayers {
            ...
            if let _ = newMatchingCharacters[index] {
                ...
                CLMLayerAnimation.animation(textLayer, duration:0.3, delay:TimeInterval(index)*0.01, animations: {
    ...
    func animateOut(_ completion: ((_ finished: Bool) -> Void)? = nil) {
        ...
        for textLayer in oldCharacterTextLayers {
            ...
            if let newMatchingIndex = oldMatchingCharacters[index] {
                let glyphIndex = layoutManager.glyphIndexForCharacter(at: newMatchingIndex)
    ...
    func matchingCharacterRanges(_ newString: String) -> (oldMatches: Dictionary<Int, Int>, newMatches: Dictionary<Int, Int>) {
        ...
        for characterLayer in oldCharacterTextLayers {
            ...
            for newCharacterLayer in characterTextLayers[startingInnerIndex...characterTextLayersEndIndex] {
                ...
                if character.isEqual(to: newCharacter) {
